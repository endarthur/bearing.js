...
import { project, inverse } from '../../src/projections/equal-angle.js';
...
describe('equal-angle projection', () => {
  ...
  it('equatorial points project to primitive circle edge', () => {
    ...
    approx(Math.sqrt(px * px + py * py), 1);
  });
  ...
  it('is conformal — preserves angles at projection center', () => {
    // Two orthogonal vectors near south pole should project to ~orthogonal directions
    const dx = 0.001;
    const z = -Math.sqrt(1 - dx * dx);
    const [px1] = project([dx, 0, z]);
    const [, py2] = project([0, dx, z]);
    // Both should have same magnitude (symmetry)
